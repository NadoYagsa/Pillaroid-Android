    ...
    protected void processImage() {
        ...
        Runnable runnable = () -> {
            ...
            if (mappedRecognitions.size() != 0) {
                ...
                for (Classifier.Recognition recognition : mappedRecognitions) {
                    if (recognition.getTitle().equals("pill")) {    // pill로 인식한 것 중 가장 신뢰율이 높은 결과에 대해 음성 가이드
                        ...
                        if (location.top < boundaryHeight) {
                            ...
                            tts.speak("알약이 너무 위에 있습니다. 손바닥을 조금만 아래로 내려주세요.", TextToSpeech.QUEUE_ADD, null, null);
                        } else if (location.bottom > frameHeight - boundaryHeight) {
                            isNormal = false;
                            Log.e("Object-Detection-result", "too under");
                            tts.speak("알약이 너무 아래에 있습니다. 손바닥을 조금만 위로 올려주세요.", TextToSpeech.QUEUE_ADD, null, null);
                        } else if (location.left < boundaryWidth) {
                            isNormal = false;
                            Log.e("Object-Detection-result", "too left");
                            tts.speak("알약이 너무 왼쪽에 있습니다. 손바닥을 조금만 오른쪽으로 이동해 주세요.", TextToSpeech.QUEUE_ADD, null, null);
                        } else if (location.right > frameWidth - boundaryWidth) {
                            isNormal = false;
                            Log.e("Object-Detection-result", "too right");
                            tts.speak("알약이 너무 오른쪽에 있습니다. 손바닥을 조금만 왼쪽으로 이동해 주세요.", TextToSpeech.QUEUE_ADD, null, null);
                        }
                        ...
                        if (objectWidth < 30 || objectHeight < 30) {
                            ...
                            tts.speak("알약이 너무 멀리 있습니다. 손바닥을 조금만 가까이 대주세요.", TextToSpeech.QUEUE_ADD, null, null);
                        } else if (objectWidth > 300 || objectHeight > 300) {
                            isNormal = false;
                            Log.e("Object-Detection-result", "too close");
                            tts.speak("알약이 너무 가까이에 있습니다. 손바닥을 조금만 멀리 대주세요.", TextToSpeech.QUEUE_ADD, null, null);
                        }

                        if (isNormal == true) {
                            Log.i("Object-Detection-result", "normal");
                            tts.speak("알약이 잘 인식되었습니다. 인식 결과를 가져오는 중입니다. 조금만 기다려주세요.", TextToSpeech.QUEUE_ADD, null, null);
                ...
                tts.speak("알약이 인식되지 않습니다. 알약을 포착할 수 있도록 카메라를 더 멀리 이동해주세요.", TextToSpeech.QUEUE_ADD, null, null);
            }
        };
